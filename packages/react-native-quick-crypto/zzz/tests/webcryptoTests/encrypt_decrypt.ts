...
describe('subtle - encrypt / decrypt', () => {
  ...
  async function testRSADecryption({
    ...
    if (privateKey !== undefined) {
      ...
      // @ts-expect-error - ciphercopy[0] is possibly undefined
  ...
  async function testRSAEncryption(
    ...
    if (modify) {
      // @ts-expect-error - plaintext[0] is possibly undefined
  ...
  async function testAESEncrypt({
    ...
    // @ts-expect-error - plaintextBuffer[0] is possibly undefined
    ...
    // @ts-expect-error - check[0] is possibly undefined
    check[0] = 255 - check[0]
