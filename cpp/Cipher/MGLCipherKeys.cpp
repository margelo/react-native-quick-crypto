...
ParseKeyResult ParsePrivateKey(EVPKeyPointer* pkey,
  ...
  //  // contain a null pointer, which OpenSSL will ignore, causing it to invoke
  //  its
  //  // default passphrase callback, which would block the thread until the
  //  user
  //  // manually enters a passphrase. We could supply our own passphrase
  //  callback
  ...
  //    if (pass == nullptr) {
  //      // OpenSSL will not actually dereference this pointer, so it can be
  //      any
  //      // non-null pointer. We cannot assert that directly, which is why we
  //      // intentionally use a pointer that will likely cause a segmentation
  //      fault
  //      // when dereferenced.
  //      CHECK_EQ(pass_len, 0);
    ...
    if (result.output_key_object_) {
      ...
      if (context != kKeyContextInput) {
        if (arguments[*offset].isString()) {
          auto cipher_name =
              arguments[*offset].getString(runtime).utf8(runtime);
      ...
      if (CheckIsArrayBuffer(runtime, arguments[*offset])) {
        ...
        if (CheckSizeInt32(runtime, passphrase)) {
          throw new jsi::JSError(runtime, "passphrase is too long");
        }
  ...
  ManagedEVPPKey ManagedEVPPKey::GetPublicOrPrivateKeyFromJs(
    ...
    if (args[*offset].asObject(runtime).isArrayBuffer(runtime)) {
      ...
      if (!CheckSizeInt32(runtime, dataArrayBuffer)) {
        throw new jsi::JSError(runtime, "data is too big");
      }
      ...
      if (config.format_ == kKeyFormatPEM) {
        ...
        ret =
            ParsePublicKeyPEM(&pkey, (const char*)dataArrayBuffer.data(runtime),
      ...
      throw new jsi::JSError(runtime,
                             "publicEncrypt api only support ArrayBuffer "
