...
import { COVERAGE_DATA, CoverageItem, CapabilityStatus } from '../data/coverage';
import { CheckCircle2, XCircle, AlertTriangle, MinusCircle, SearchX } from 'lucide-react';
...
export function CoverageTable() {
    ...
    const deriveStatus = (item: CoverageItem): CapabilityStatus => {
        ...
        const allImplemented = subStatuses.every(s => s === 'implemented');
        const allMissing = subStatuses.every(s => s === 'missing');
        const allNotInNode = subStatuses.every(s => s === 'not-in-node');

        if (allImplemented) return 'implemented';
        if (allMissing) return 'missing';
        if (allNotInNode) return 'not-in-node';
    ...
    const filterProcessedItems = (items: CoverageItem[]): CoverageItem[] => {
        return items
            .map((item) => {
                ...
                const subMatches = item.subItems ? filterProcessedItems(item.subItems) : [];
    ...
    const filteredCategories = processedData.map(category => ({
        ...category,
            ...
            {filteredCategories.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed border-fd-border rounded-xl bg-fd-secondary/10">
                    ...
                    <p className="text-sm text-fd-muted-foreground mt-1">
                        No results matching "<span className="font-medium text-fd-foreground">{search}</span>"
                    </p>
                    <button
                        onClick={() => setSearch('')}
                    ...
                    <div key={category.title} className="bg-fd-card rounded-xl border border-fd-border overflow-hidden">
                        ...
                        <div className="divide-y divide-fd-border/50">
                            {category.items.map((item) => (
                                <div key={item.name} className="px-6 py-3 flex items-start justify-between hover:bg-fd-secondary/10 transition-colors">
                                        ...
                                        {item.subItems && item.subItems.length > 0 && (
                                            <div className="mt-2 ml-4 pl-4 border-l border-fd-border/50 space-y-2">
                                                {item.subItems.map(sub => (
                                                    <div key={sub.name} className="flex items-center gap-2 text-sm text-fd-muted-foreground">
