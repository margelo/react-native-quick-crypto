...
<Callout type="warn" title="Single Use">
  Cipher instances are single-use. Calling `update()` or `final()` after `final()` throws an error. Create a new cipher instance for each encryption operation. When using string output encoding (e.g. `'base64'`), the encoding must be consistent between `update()` and `final()` calls.
</Callout>
