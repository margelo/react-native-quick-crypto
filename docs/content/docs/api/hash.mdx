...
One-shot hashing function. Computes a digest in a single call without creating a `Hash` object.
...
const buf = hash('sha512', Buffer.from([1, 2, 3]));
```
...
| **SHA3-256/384/512** | 256/384/512 bits | ✅ **Secure** | Fixed-output SHA-3 variants. |
| **SHAKE128/256** | Variable | ✅ **Secure** | XOF (Extendable Output) via cSHAKE in WebCrypto. |
