  ...
  // eslint-disable-next-line no-lone-blocks
  {
    ...
    const testImportJwk = async (
      ...
      if (extractable) {
        ...
        expect(pubJwk.key_ops).to.have.all.members(publicUsages);
        expect(pubJwk.ext).to.equal(true);
        expect(pubJwk.kty).to.equal('EC');
        expect(pubJwk.x).to.equal(jwk.x);
        expect(pubJwk.y).to.equal(jwk.y);
        expect(pubJwk.crv).to.equal(jwk.crv);

        expect(pvtJwk.key_ops).to.have.all.members(privateUsages);
        expect(pvtJwk.ext).to.equal(true);
        expect(pvtJwk.kty).to.equal('EC');
        expect(pvtJwk.x).to.equal(jwk.x);
        expect(pvtJwk.y).to.equal(jwk.y);
        expect(pvtJwk.crv).to.equal(jwk.crv);
