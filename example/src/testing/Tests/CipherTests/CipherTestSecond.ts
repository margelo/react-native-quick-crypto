...
describe('createCipheriv/createDecipheriv', () => {
  ...
  function testCipher1(key: string | Buffer, iv: string | Buffer) {
    it('testCipher1 + ' + key + ' + ' + iv, () => {
      ...
      const plaintext =
        ...
        'jAfaFg**';
      const cipher = QuickCrypto.createCipheriv('des-ede3-cbc', key, iv);
      let ciph = cipher.update(plaintext, 'utf8', 'hex');
      ciph += cipher.final('hex');

      const decipher = QuickCrypto.createDecipheriv('des-ede3-cbc', key, iv);
      ...
      const cStream = QuickCrypto.createCipheriv('des-ede3-cbc', key, iv);
      cStream.end(plaintext);
      ciph = cStream.read();

      const dStream = QuickCrypto.createDecipheriv('des-ede3-cbc', key, iv);
      dStream.end(ciph);
  ...
  function testAESGCM(key: Buffer, iv: Buffer) {
    ...
    it('AES-GCM with key and iv - default AuthTag length', async () => {
      const defaultCipher = QuickCrypto.createCipheriv('aes-256-gcm', key, iv);
