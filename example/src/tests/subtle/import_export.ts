...
import type {
  ...
  RandomTypedArrays,
  RSAKeyPairAlgorithm,
  // SubtleAlgorithm, // TODO: for 'bad usages' test
...
test(SUITE, 'Good Input - Uint8Array', async () => {
  await subtle.importKey(
    'raw',
    new Uint8Array([
...
test(SUITE, ' importKey - raw - pbkdf2 - empty byte source #735', async () => {
...
test(SUITE, 'AES import raw / export raw', async () => {
  const rawKeyData = getRandomValues(new Uint8Array(32));
...
test(SUITE, 'importKey, raw, AES-GCM string algo', async () => {
  const rawKeyData = getRandomValues(new Uint8Array(32));
  const keyData = binaryLikeToArrayBuffer(rawKeyData);
...
const testFn = (rawKeyData: RandomTypedArrays, descr: string): void => {
  test(SUITE, `AES import raw / export jwk (${descr})`, async () => {
    const keyData = binaryLikeToArrayBuffer(rawKeyData);
...
testFn(random as Uint8Array, 'random');

// test while ensuring at least one of the elements is zero
const withZero = getRandomValues(new Uint8Array(32));
withZero[4] = 0;
...
test(SUITE, 'EC import raw / export spki (osp)', async () => {
  const key = await subtle.importKey(
    'raw',
    base64ToArrayBuffer(
...
// from Node.js https://github.com/nodejs/node/blob/main/test/parallel/test-webcrypto-export-import-ec.js
{
  ...
  const testImportJwk = async (
    ...
    if (extractable) {
      ...
      expect(pvtJwk.key_ops).to.have.all.members(privateUsages, 'pvt key_ops');
  ...
  for (const vector of testVectors) {
    for (const namedCurve of curves) {
      ...
      test(SUITE, `EC raw, ${vector.name}, ${namedCurve}`, async () => {
...
test(SUITE, 'HMAC should import raw HMAC key', async () => {
  const keyData = getRandomValues(new Uint8Array(32));
  const key = await subtle.importKey(
    'raw',
    keyData,
...
test(SUITE, 'HMAC should import JWK HMAC key', async () => {
  const jwk: JWK = {
    kty: 'oct',
...
test(SUITE, 'HMAC should reject invalid key usages', async () => {
  const keyData = getRandomValues(new Uint8Array(32));
...
test(SUITE, 'HMAC should reject invalid JWK format', async () => {
...
test(SUITE, 'HMAC should reject invalid key length', async () => {
  const jwk: JWK = {
    kty: 'oct',
    k: 'Y0zt37HgOx-BY7SQjYVmrqhPkO44Ii2Jcb9yydUDPfE',
    alg: 'HS256',
...
test(SUITE, 'HMAC should reject invalid zero key length', async () => {
  const jwk: JWK = {
    kty: 'oct',
    k: 'Y0zt37HgOx-BY7SQjYVmrqhPkO44Ii2Jcb9yydUDPfE',
    alg: 'HS256',
...
test(SUITE, 'HMAC should reject invalid keyData', async () => {
  await assertThrowsAsync(
    async () =>
      await subtle.importKey(
        'jwk',
...
test(SUITE, 'HMAC should reject unsupported import format', async () => {
  const keyData = getRandomValues(new Uint8Array(32));
  ...
  hashes.forEach(hash => {
    [true, false].forEach(extractable => {
      testVectors.forEach(vector => {
        test(
          SUITE,
          `rsa importKey spki ${vector.name} ${size} ${hash} ${extractable}`,
          async () => {
            await testImportSpki(vector, size, hash, extractable);
          },
        );
        // test(SUITE, `rsa importKey pkcs8 ${vector.name} ${size} ${hash} ${extractable}`, async () => {
        //   await testImportPkcs8(vector, size, hash, extractable);
        // });
        // test(SUITE, `rsa importKey jwk ${vector.name} ${size} ${hash} ${extractable}`, async () => {
...
// TODO: re-enable after createPublicKey/createPrivateKey are ported from 0.x
// {
...
//   };
//   for (const [name, [publicUsage, privateUsage]] of Object.entries(badUsages)) {
//     test(
//       SUITE,
//       `bad usages ${name} ${publicUsage} ${privateUsage}`,
//       async () => {
//         await assertThrowsAsync(
//           async () =>
...
for (const variant of MLKEM_VARIANTS) {
  test(SUITE, `${variant} export raw-public - correct size`, async () => {
    ...
    expect(rawPub.byteLength).to.equal(MLKEM_PUBLIC_KEY_SIZES[variant]);
  });

  test(SUITE, `${variant} export raw-seed - 64 bytes`, async () => {
    const keyPair = (await subtle.generateKey({ name: variant }, true, [
      'encapsulateBits',
      'decapsulateBits',
    ])) as CryptoKeyPair;
    ...
    expect(seed.byteLength).to.equal(MLKEM_SEED_SIZE);
  });

  test(
    SUITE,
    `${variant} roundtrip: export raw-public -> import -> encapsulate`,
    async () => {
      const keyPair = (await subtle.generateKey({ name: variant }, true, [
        'encapsulateBits',
        'decapsulateBits',
      ...
      expect(sharedKey.byteLength).to.equal(32);
      expect(ciphertext.byteLength).to.be.greaterThan(0);
      ...
      expect(new Uint8Array(decapsulated)).to.deep.equal(
  ...
  test(
    SUITE,
    `${variant} roundtrip: export raw-seed -> import -> decapsulate`,
    async () => {
      const keyPair = (await subtle.generateKey({ name: variant }, true, [
        'encapsulateBits',
        'decapsulateBits',
  ...
  test(SUITE, `${variant} raw-public re-export matches original`, async () => {
    const keyPair = (await subtle.generateKey({ name: variant }, true, [
      'encapsulateBits',
      'decapsulateBits',
    ])) as CryptoKeyPair;

    const rawPub1 = (await subtle.exportKey(
      'raw-public',
      keyPair.publicKey as CryptoKey,
    )) as ArrayBuffer;
    ...
    const rawPub2 = (await subtle.exportKey(
      'raw-public',
      imported,
    )) as ArrayBuffer;

    expect(Buffer.from(rawPub2).toString('hex')).to.equal(
      Buffer.from(rawPub1).toString('hex'),
    );
  });
}
...
test(SUITE, 'ML-KEM-768 unwrapKey with AES-KW', async () => {
  ...
  const mlkemKeyPair = (await subtle.generateKey({ name: 'ML-KEM-768' }, true, [
    'encapsulateBits',
    'decapsulateBits',
  ])) as CryptoKeyPair;
  ...
  expect(unwrapped.type).to.equal('public');
  expect(unwrapped.algorithm.name).to.equal('ML-KEM-768');
  ...
  const decapsulated = await subtle.decapsulateBits(
    { name: 'ML-KEM-768' },
    mlkemKeyPair.privateKey as CryptoKey,
    ciphertext,
  );
